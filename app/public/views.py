...
def member_page_function(username="admin"):
    ...
    # 笔记分页
    note_count = Note.objects(
        public_status=NOTECONSTANTS.PUBLIC, belong=this_user).count()
    page_count = note_count / NOTECONSTANTS.PER_PAGE_COUNT + \
        1 if note_count % NOTECONSTANTS.PER_PAGE_COUNT else note_count / \
        NOTECONSTANTS.PER_PAGE_COUNT
    if note_count == 0:
        page_count += 1
    ...
    now_page_note = Note.objects(public_status=NOTECONSTANTS.PUBLIC, belong=this_user).skip(
        NOTECONSTANTS.PER_PAGE_COUNT * (now_page - 1)).limit(NOTECONSTANTS.PER_PAGE_COUNT)
    # 用户笔记公开分类
...
def member_cate_function(username, cate):
    ...
    # 笔记分页
    note_count = Note.objects(
        public_status=NOTECONSTANTS.PUBLIC, belong=this_user, public_cate=this_cate).count()
    page_count = note_count / NOTECONSTANTS.PER_PAGE_COUNT + \
        1 if note_count % NOTECONSTANTS.PER_PAGE_COUNT else note_count / \
        NOTECONSTANTS.PER_PAGE_COUNT
    if note_count == 0:
    ...
    # 当页笔记
    now_page_note = Note.objects(public_status=NOTECONSTANTS.PUBLIC, belong=this_user, public_cate=this_cate).skip(
        NOTECONSTANTS.PER_PAGE_COUNT * (now_page - 1)).limit(NOTECONSTANTS.PER_PAGE_COUNT)
