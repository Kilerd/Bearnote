...
class RegisterForm(Form):
    ...
    password = PasswordField('Password', validators=[DataRequired(), Length(6, 18), InputRequired()])
    confirm_password = PasswordField('Repeat Password', validators=[DataRequired(), InputRequired(), EqualTo('password', message='Passwords must match')])
...
class SettingResetPasswordForm(Form):
    ...
    confirm_newpassword = PasswordField(u'重复新密码', validators=[DataRequired(),InputRequired(),EqualTo('newpassword', message='Passwords must match')])
